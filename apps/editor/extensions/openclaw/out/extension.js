...
const home_1 = require("./panels/home");
const status_1 = require("./panels/status");
const config_1 = require("./panels/config");
const DEFAULT_GATEWAY_PORT = 18789;
function getConfiguredGatewayPort() {
    try {
...
async function hideActivityBarItems(context) {
    ...
    try {
        ...
        const existing = config.get('workbench.activityBar.pinnedViewContainers') ?? [];
        ...
        await config.update('workbench.activityBar.pinnedViewContainers', authoritative, vscode.ConfigurationTarget.Global);
...
async function openOpenClawFolder() {
    ...
    if (!fs.existsSync(openclawPath)) {
        ...
        try {
            if (fs.existsSync(staleWorkspaceFile)) {
                fs.unlinkSync(staleWorkspaceFile);
            }
        }
        catch { /* non-fatal */ }
        return;
...
function initBalanceBar(context) {
    // Restore cached backend balance so status bar shows the correct value immediately on startup
    const cachedBackendBalance = context.globalState.get(BACKEND_BALANCE_KEY, null);
    let backendBalance = cachedBackendBalance;
    let displayedBalance = cachedBackendBalance; // tracks what's visually shown
    let animTimer;
    ...
    function renderBalance(value, counting = false) {
        ...
        try {
            home_1.HomePanel.currentPanel?.postBalanceUpdate(value);
        }
        catch { /* non-fatal */ }
    }
    ...
    function startCountdown() {
        stopCountdown();
        if (burnRatePerMs <= 0 || displayedBalance === null)
            return;
    ...
    function updateBurnRate(newBalance) {
        const now = Date.now();
    ...
    async function fetchAndUpdateBackendBalance() {
        try {
            ...
            let jwt = context.globalState.get(OCC_JWT_KEY, '');
            if (!jwt) {
                try {
                    const legacyJwt = await vscode.commands.executeCommand('occ.auth.getLegacyJwt');
                    if (legacyJwt) {
                        jwt = legacyJwt;
            ...
            if (r.ok) {
                ...
                const currentJwt = context.globalState.get(OCC_JWT_KEY, '');
                if (currentJwt !== jwt) {
                    return;
                }
                ...
                if (!context.globalState.get(OS_REPORTED_KEY)) {
                    ...
                    fetch('https://occ.mba.sh/api/v1/me/platform', {
                        ...
                        if (resp.ok) {
                            void context.globalState.update(OS_REPORTED_KEY, true);
                        }
                    }).catch(() => { });
                }
            }
    ...
    function startBackendPolling() {
        if (backendPollTimer)
            clearInterval(backendPollTimer);
        backendPollTimer = setInterval(() => void fetchAndUpdateBackendBalance(), 5000);
    }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('openclaw.balance.spend', (_amount) => {
        ...
        if (backendBalance !== null) {
            vscode.window.showInformationMessage(`OCC Credits · $${backendBalance.toFixed(4)} remaining`, 'Get More Credits').then(sel => {
    ...
    vscode.commands.registerCommand('openclaw.jwt.set', async (token) => {
    ...
    vscode.commands.registerCommand('openclaw.smokeTest', async () => {
        ...
        try {
            ...
            log('    occLegacyJwt check: renderer not ready');
        }
        if (!jwt) {
            ...
            const inferenceTest = async (label, key) => {
                ...
                try {
                    ...
                    if (resp.ok) {
                        try {
                            ...
                            log(`    (response is not JSON — likely SSE stream)`);
                        }
                    }
                }
                catch (e) {
            ...
            // 3. MoltPilot key
            if (moltpilotKey) {
                await inferenceTest('MoltPilot key', moltpilotKey);
            }
            else {
            ...
            // 4. OCC key
            if (occKey) {
                await inferenceTest('OCC key', occKey);
            }
            else {
    ...
    }), { dispose: () => { if (backendPollTimer)
            clearInterval(backendPollTimer); stopCountdown(); } });
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('openclaw.home', () => {
        home_1.HomePanel.createOrShow(context.extensionUri);
    }), vscode.commands.registerCommand('openclaw.configure', async () => {
        const reachable = await isWebServerReachable();
        if (reachable) {
            const dashInfo = (0, config_1.getDashboardUrl)();
            ...
            const message = `The OpenClaw web configuration server is not running at ${configUrl}.\n\n` +
        ...
        try {
            ...
            vscode.window.showErrorMessage('openclaw.json not found at ~/.openclaw/openclaw.json. Please create it first.');
            return;
        }
        const message = `Please review my openclaw.json configuration below, identify any issues ` +
        ...
        void home_1.HomePanel.runInstall(context.extensionUri, process.platform, process.arch, process.env.SHELL ?? '');
        ...
        status_1.StatusPanel.createOrShow(context.extensionUri);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('openclaw.runWithSudo', async (command, reason) => {
        ...
        if (!password)
            return { result: 'User cancelled the password prompt.', exitCode: 1 };
        return new Promise(resolve => {
            ...
            child.on('close', (code) => resolve({ result: out.trim() || '(no output)', exitCode: code ?? 1 }));
            child.on('error', (err) => resolve({ result: err.message, exitCode: 1 }));
        });
    }));
    // ── Anonymous install ping (fires once per install, no personal data) ──
    const PING_KEY = 'aptabasePingedV1';
    if (!context.globalState.get(PING_KEY)) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('openclaw.getSystemStatus', async () => {
        ...
        const gatewayRunning = await new Promise(resolve => {
        ...
        const hasAiModel = installed && !!(config['auth_choice'] || config['openai_api_key'] || config['anthropic_api_key'] ||
        ...
        if (channelsRaw && typeof channelsRaw === 'object' && !Array.isArray(channelsRaw)) {
            channelNames = Object.entries(channelsRaw)
                .filter(([, v]) => {
                if (!v)
                    return false;
                // If the channel is an object, require enabled !== false
                if (typeof v === 'object' && v['enabled'] === false)
                    return false;
                return true;
            })
                .map(([k]) => k);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('occ.onboarding.chooseMoltPilot', async () => {
        ...
        if (pick) {
            ...
            vscode.window.showInformationMessage(`${pick} selected — you'll enter your API key when you install OpenClaw from OCC Home.`);
        }
    }), vscode.commands.registerCommand('occ.onboarding.darkTheme', async () => {
        await vscode.workspace.getConfiguration('workbench').update('colorTheme', 'OpenClaw Dark', vscode.ConfigurationTarget.Global);
    }), vscode.commands.registerCommand('occ.onboarding.lightTheme', async () => {
        await vscode.workspace.getConfiguration('workbench').update('colorTheme', 'OpenClaw Light', vscode.ConfigurationTarget.Global);
    }));
