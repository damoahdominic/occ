...
class OnboardingPanel {
    ...
    static showIfNeeded(context, extensionUri) {
        const done = context.globalState.get(OnboardingPanel.ONBOARDING_DONE_KEY) ?? false;
        if (done) {
            return false;
        }
    ...
    static _show(context, extensionUri) {
        if (OnboardingPanel._panel) {
            OnboardingPanel._panel.reveal();
            return;
        }
        const panel = vscode.window.createWebviewPanel('occOnboarding', 'Welcome to OCC', vscode.ViewColumn.One, { enableScripts: true, localResourceRoots: [vscode.Uri.joinPath(extensionUri, 'media')] });
        OnboardingPanel._panel = panel;
        const iconUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'icon.png'));
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            if (msg.command === 'setTheme') {
                ...
                const aiPreference = msg.aiPreference;
                if (aiPreference) {
    ...
    static _getHtml(iconUri) {
        ...
        const providerCards = providers.map(p => `<button class="prov-card" data-id="${p.id}" onclick="pickProvider(this)">
...
OnboardingPanel.ONBOARDING_DONE_KEY = 'occ.onboardingDone';
OnboardingPanel.AI_PREFERENCE_KEY = 'occ.aiPreference';
