...
exports.getOrStartConfigProxy = getOrStartConfigProxy;
exports.stopConfigProxy = stopConfigProxy;
...
function getOrStartConfigProxy(targetPort = DEFAULT_GATEWAY_PORT) {
    // Restart proxy if target port changed
    if (_proxyServer && _proxyPort && _proxyTargetPort === targetPort)
        return Promise.resolve(_proxyPort);
    if (_proxyServer) {
        _proxyServer.close();
...
function getDashboardUrl() {
    try {
        ...
        const token = auth?.['mode'] === 'token' && typeof auth?.['token'] === 'string' ? auth['token'] : '';
        const url = token
...
class ConfigPanel {
    ...
    static async createOrShow() {
        ...
        const panel = vscode.window.createWebviewPanel('openclawConfig', 'OpenClaw Configure', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    _iframeHtml(src, externalSrc) {
        // Lucide SVG icon helper — inline paths, no CDN needed.
        const icon = (paths) => `<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
