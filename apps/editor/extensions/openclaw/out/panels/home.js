...
class HomePanel {
    constructor(panel, extensionUri) {
        ...
        this._sidebarOpen = false; // tracks chat sidebar open state across webview reloads
        this._pollTick = 0;
        this._lastJwt = '';
        this._lastInstalledVersion = null;
        this._autoUpdateTriggered = false; // fire at most once per panel session
        this._closeSidebarOnGatewayStart = false; // close sidebar once gateway reaches running after first install
        this._panel = panel;
        this._extensionUri = extensionUri;
        this._outputChannel = vscode.window.createOutputChannel('OpenClaw Gateway');
        const iconUri = this._panel.webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'icon.png'));
        ...
        const configWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.Uri.file(path.join(os.homedir(), '.openclaw')), 'openclaw.json'), false, true, false);
        ...
        const homeWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.Uri.file(os.homedir()), '.openclaw'), false, true, false);
        ...
        this._panel.webview.onDidReceiveMessage(msg => {
            if (msg.command === 'gatewayAction') {
                void this._handleGatewayAction(msg.action);
            }
            else if (msg.command === 'checkVersion') {
                void this._checkLatestVersion();
            }
            else if (msg.command === 'runUpdate') {
                void vscode.commands.executeCommand('void.openChatWithMessage', 'Please run `openclaw update` to upgrade OpenClaw to the latest version.', 'agent');
            }
            else if (msg.command === 'runSetup') {
                void this._runSetup(msg);
            }
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        vscode.commands.executeCommand('void.openChatWithMessage', 'Run `openclaw gateway start` to start the OpenClaw gateway.', 'agent');
                    }, 1000);
                }, 500);
            }
                ...
                HomePanel._pendingPasswordResolve?.(msg.password);
                ...
                void vscode.commands.executeCommand(cmd).then(async () => {
                    ...
                    this._sidebarOpen = await vscode.commands.executeCommand('void.sidebar.isVisible').then(v => !!v, () => this._sidebarOpen);
                    try {
                        this._panel.webview.postMessage({ type: 'chatState', open: this._sidebarOpen });
                    }
                    catch { }
                });
            }
                ...
                if (!fs.existsSync(filePath)) {
                    if (file === 'MEMORY.md') {
                        ...
                        vscode.window.showWarningMessage(`${file} not found in ${workspaceDir}. OpenClaw may not have initialised its workspace yet.`);
                        return;
                    }
                }
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('openclawHome', 'OCC Home', vscode.ViewColumn.One, { enableScripts: true, localResourceRoots: [
    ...
    postBalanceUpdate(amount) {
        try {
            this._panel.webview.postMessage({ type: 'balanceUpdate', amount });
        }
        catch { /* non-fatal */ }
    }
     ...
     */
    static async runInstall(extensionUri, platform, arch, shell) {
        HomePanel.createOrShow(extensionUri);
        const panel = HomePanel.currentPanel;
        if (!panel)
            return;
        const post = (msg) => { try {
            panel._panel.webview.postMessage(msg);
        }
        catch { } };
        let fullLog = '';
        const tee = (text) => { fullLog += text; post({ type: 'installLog', text }); writeLog(text); };
        writeLog(`\n=== runInstall START platform=${platform} arch=${arch} ===\n`);
        post({ type: 'installState', state: 'running' });
        const env = panel._buildExecEnv();
        const isPermError = (s) => /EACCES|permission denied|EPERM|not permitted|Need sudo access|needs to be an Administrator/i.test(s);
        // Quick command check helper (no output, just exit code)
        const cmdExists = (cmd) => new Promise(resolve => cp.exec(cmd, { env, timeout: 5000, windowsHide: true }, err => resolve(!err)));
        ...
        const captureInstalledPath = async () => {
            if (platform === 'win32')
                return; // Windows uses cmd shims; handled by _testOpenClawCli
            const candidates = [];
            // 1. Ask npm where its global prefix is (same env as the install used)
            const prefixResult = await new Promise(resolve => {
                cp.exec('npm config get prefix', { env, timeout: 5000 }, (err, stdout) => resolve(err ? '' : (stdout || '').trim()));
            });
            if (prefixResult)
                candidates.push(path.join(prefixResult, 'bin', 'openclaw'));
            // 2. Hard-coded common macOS/Linux paths
            const home = os.homedir();
            candidates.push('/usr/local/bin/openclaw', '/opt/homebrew/bin/openclaw', path.join(home, '.local', 'bin', 'openclaw'), path.join(home, '.npm-global', 'bin', 'openclaw'), path.join(home, '.openclaw', 'bin', 'openclaw'));
        ...
        if (platform === 'darwin') {
            ...
            const xcodeOk = await new Promise(resolve => cp.exec('xcode-select -p', { env, timeout: 5000 }, (err, stdout) => {
        ...
        if (platform !== 'win32') {
            let needsSudo = false;
            if (npmOk) {
                const prefixResult = await new Promise(resolve => cp.exec('npm config get prefix', { env, timeout: 5000 }, (err, stdout) => resolve(err ? '' : stdout?.toString().trim() || '')));
                if (prefixResult) {
                    try {
                        const gBin = path.join(prefixResult, 'bin');
                        const gLib = path.join(prefixResult, 'lib');
                        if (fs.existsSync(gBin))
                            fs.accessSync(gBin, fs.constants.W_OK);
                        if (fs.existsSync(gLib))
                            fs.accessSync(gLib, fs.constants.W_OK);
                    }
                    catch {
                        needsSudo = true;
                    }
            ...
            if (needsSudo) {
                ...
                if (!sudoOk) {
                    tee('  Incorrect password or cancelled.\n');
                    failCancelled();
                    return;
                }
        ...
        if (npmOk) {
            ...
            const spawnOpts = platform === 'win32' ? { shell: true, windowsHide: true } : {};
            ...
            if (platform === 'darwin') {
                ...
                const hasCmdSync = (cmd) => {
                    try {
                        cp.execSync(`which ${cmd}`, { env, stdio: 'ignore' });
                        return true;
                    }
                    catch {
                        return false;
                ...
                if (!pkgResult) {
                    tee('No supported package manager found (tried apt-get, dnf, yum).\n');
                    await fail();
                    return;
                }
                if (pkgResult.code !== 0) {
        ...
        if (platform === 'win32') {
            ...
            const r = await runCaptured('powershell', psArgs, { windowsHide: true });
            if (r.code === 0) {
                await succeed();
                return;
            }
    ...
    async _update() {
        ...
        const iconUri = this._panel.webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'icon.png'));
        const occJwt = await vscode.commands.executeCommand('occ.auth.getLegacyJwt').then(r => r ?? '', () => '');
        ...
        if (!isConfigured) {
            ...
            const emojiBaseUri = this._panel.webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'emojis')).toString();
            // Read AI model info from openclaw.json
            let aiModelName = '';
            try {
                const raw = fs.readFileSync(configFile, 'utf-8');
                const cfg = JSON.parse(raw);
                const primaryModel = cfg
                ...
                if (primaryModel) {
                    ...
                    const modelDef = providerModels.find((m) => m.id === modelId);
        ...
        if (isInstalled) {
            setTimeout(() => {
                try {
                    this._panel.webview.postMessage({ type: 'autoCheckVersion' });
                }
                catch { }
                void this._checkLatestVersion();
            }, 800);
    ...
    _startPolling() {
        ...
        const tick = async () => {
            ...
            const [status, aiRunning, sidebarVisible, jwt] = await Promise.all([
                ...
                vscode.commands.executeCommand('void.getIsRunning').then(v => !!v, () => false),
                vscode.commands.executeCommand('void.sidebar.isVisible').then(v => !!v, () => this._sidebarOpen),
                vscode.commands.executeCommand('occ.auth.getLegacyJwt').then(r => r ?? '', () => ''),
            ]);
            this._sidebarOpen = sidebarVisible;
            // Don't overwrite the intermediary status while a gateway command is in progress.
            if (!this._commandAction) {
                try {
                    this._panel.webview.postMessage({ type: 'gatewayStatus', status });
                }
                catch { }
            }
            ...
            try {
                this._panel.webview.postMessage({ type: 'aiRunning', running: aiRunning });
            }
            catch { }
            try {
                this._panel.webview.postMessage({ type: 'chatState', open: this._sidebarOpen });
     ...
     */
    async _handleGatewayAction(action) {
        const intermediary = action === 'start' ? 'starting' : action === 'stop' ? 'stopping' : 'restarting';
        const expectedState = action === 'stop' ? 'stopped' : 'running';
        this._commandAction = action;
        try {
            this._panel.webview.postMessage({ type: 'gatewayStatus', status: intermediary });
        }
        catch { }
    ...
    _pollUntilState(expected, intermediary, maxWaitMs = 180000) {
        const deadline = Date.now() + maxWaitMs;
        const tick = async () => {
            if (Date.now() > deadline) {
                this._commandAction = null;
                try {
                    this._panel.webview.postMessage({ type: 'gatewayStatus', status: await this._checkGatewayStatus() });
                }
                catch { }
                return;
            }
    ...
    _fetchLatestVersion() {
        return new Promise(resolve => {
            ...
            const req = https.get({ hostname: 'registry.npmjs.org', path: '/openclaw/latest', headers: { Accept: 'application/json' } }, res => {
                let data = '';
    ...
    async _checkLatestVersion() {
        const post = (html) => {
            try {
                this._panel.webview.postMessage({ type: 'versionResult', html });
            }
            catch { }
        };
        ...
        if (norm(installed) === norm(latest)) {
            ...
            post(`<span style="color:#fbbf24">Update available: <strong>${latest}</strong> &mdash; you have <strong>${installed}</strong>.</span>` +
    ...
    async _autoUpdateIfOutdated() {
        try {
            const [cliCheck, latest] = await Promise.all([
                this._testOpenClawCli(),
                this._fetchLatestVersion(),
            ]);
            if (!cliCheck.ok || !latest)
                return;
            const norm = (v) => { const m = v.match(/\d+\.\d+(?:\.\d+)*/); return m ? m[0] : v.replace(/^v/i, '').split(/[-+(]/)[0].trim(); };
            const installed = (cliCheck.output ?? '').trim();
            if (norm(installed) === norm(latest))
                return; // already up to date
            void vscode.commands.executeCommand('void.openChatWithMessage', `OpenClaw is installed but version ${installed} is not the latest (${latest}). Please update it now.\n\n` +
                `Run: openclaw update --yes --non-interactive\n\n` +
                `If that command is not available, use: npm install -g openclaw@latest\n\n` +
                `After updating, verify with: openclaw --version`, 'agent');
        }
        catch { /* best-effort */ }
    }
    _getLoadingHtml(iconUri) {
        return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <div class="spinner-wrap">
    ...
    // ── Setup wizard ───────────────────────────────────────────────────────────
    async _runSetup(data) {
        const post = (msg) => { try {
            this._panel.webview.postMessage(msg);
        }
        catch { } };
        const wizardPost = (text, done, ok) => {
            writeLog(text);
        ...
        if (process.platform !== 'win32') {
            const nodeVerRaw = await new Promise(resolve => {
                cp.exec('node --version', { env, timeout: 5000 }, (err, stdout) => resolve((stdout || '').toString().trim()));
            });
            ...
            if (nodeMinor < 22) {
                ...
                if (fs.existsSync(nvmSh)) {
                    ...
                    const nvmR = await new Promise(resolve => {
                        const child = cp.spawn('bash', ['-c',
                            `. "${nvmSh}" && nvm install 22 && nvm use 22 && nvm alias default 22`
                        ], { env, stdio: ['ignore', 'pipe', 'pipe'] });
                        child.stdout?.on('data', (d) => wizardPost(d.toString(), false, false));
                        child.stderr?.on('data', (d) => wizardPost(d.toString(), false, false));
                ...
                if (ok) {
                    if (isFree) {
                        ...
                        try {
                            ...
                            fs.writeFileSync(path.join(occDir, 'moltpilot-tier.json'), JSON.stringify({ tier: 'free', grantedAt: new Date().toISOString(), limitUsd: 1.00 }));
                        }
                        catch { /* non-fatal */ }
    ...
    _schedulePostUninstallClose() {
        ...
        this._uninstallCloseWatcher = setInterval(async () => {
            ...
            const isRunning = await vscode.commands.executeCommand('void.getIsRunning')
                .then(v => !!v, () => false);
            if (isRunning) {
                wasRunning = true;
                return;
    ...
    async _runUninstall() {
        ...
        setTimeout(() => {
            ...
            vscode.commands.executeCommand('void.openChatWithMessage', 'Uninstall OpenClaw. No sudo.\n1. cd $HOME\n2. pkill -9 -f openclaw\n3. openclaw uninstall --all --yes --non-interactive\nConfirm each step ran OK, then tell the user it\'s done.', 'agent');
    ...
    async _runCassSetup() {
        ...
        const runCmd = (cmd, args, opts = {}) => new Promise(resolve => {
            const child = cp.spawn(cmd, args, { env, stdio: ['ignore', 'pipe', 'pipe'], ...opts });
            let out = '';
            child.stdout?.on('data', (d) => { const s = d.toString(); out += s; post({ type: 'wizardLog', text: s, done: false, ok: false }); });
            child.stderr?.on('data', (d) => { const s = d.toString(); out += s; post({ type: 'wizardLog', text: s, done: false, ok: false }); });
...
The binary is already downloaded — do NOT re-download or compile anything.`;
    ...
    _getSetupHtml(isInstalled, iconUri, occUser = null) {
        ...
        const providerCards = providers.map(p => `<button class="prov-card" data-id="${p.id}" data-placeholder="${p.placeholder}" data-hint="${p.hint}" onclick="pickProvider(this)">
...
<body>
  ...
  <script>
    ...
    _getWizardHtml(iconUri, occUser = null) {
        // Render user area statically (avoids JS innerHTML escaping issues)
        let userAreaHtml;
        if (!occUser) {
            userAreaHtml = `<button class="sign-in-btn" onclick="signIn()">Sign In</button>`;
...
<body>
  ...
  <script>
    ...
    _getHtml(isInstalled, dirExists, cliCheck, iconUri, occJwt = '', occUser = null, emojiBaseUri = '', aiModelName = '') {
        ...
        const ic = (d, size = 13, opacity = '0.55') => `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;opacity:${opacity}">${d}</svg>`;
...
<body${isInstalled ? '' : ' class="not-installed"'}>
  ...
  <div class="pills-row">
    ${['AGENTS.md', 'IDENTITY.md', 'USER.md', 'MEMORY.md', 'SOUL.md', 'HEARTBEAT.md'].map(f => `<span class="pill" onclick="openFile('${f}')">${f}</span>`).join('\n    ')}
  </div>
  <div class="btn-group">
  ...
  <script>
    ...
    async _testOpenClawCli() {
        if (process.platform === 'win32') {
            ...
            if (cmdPath) {
                return new Promise(resolve => {
                    cp.execFile('cmd.exe', ['/c', cmdPath, '--version'], { timeout: 30000, windowsHide: true, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
                        if (error) {
        ...
        if (fs.existsSync(nvmSh)) {
            ...
            const nvmResult = await new Promise(resolve => {
                ...
                if (fs.existsSync(candidate)) {
                    const result = await new Promise(resolve => {
                        cp.execFile(candidate, ['--version'], { timeout: 10000, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
                            ...
                            if (error || !out)
                                resolve({ ok: false, error: out || error?.message, command: `${candidate} --version` });
                            else
                                resolve({ ok: true, output: out, command: `${candidate} --version` });
                        });
                    });
                    if (result.ok)
                        return result;
        ...
        return new Promise(resolve => {
            cp.execFile(cliPath, ['--version'], { timeout: 15000, maxBuffer: 1024 * 1024, env: this._buildExecEnv() }, (error, stdout, stderr) => {
                if (error) {
                    resolve({ ok: false, error: stderr?.toString().trim() || error.message || `Exit ${error.code}`, command: `${cliPath} --version` });
                }
                else {
    ...
    async _findWindowsNodeExe() {
        ...
        try {
            const found = await new Promise((resolve, reject) => cp.exec('where node.exe', { timeout: 3000, windowsHide: true }, (err, stdout) => err ? reject(err) : resolve(stdout.trim().split(/\r?\n/)[0]?.trim() || '')));
    ...
    _spawnNodeMjs(nodeExe, mjs, display) {
        return new Promise(resolve => {
    ...
    async _findOpenClawPath() {
        const cfgPath = vscode.workspace.getConfiguration('openclaw').get('cliPath');
    ...
    async _getNpmGlobalCliCandidates() {
    ...
    _resolveWindowsCliPath(candidate) {
        if (process.platform !== 'win32')
            return candidate;
    ...
    _getPreferredWindowsCmdPath(candidate) {
        if (process.platform !== 'win32')
            return candidate;
        const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
    ...
    _runCommand(cmd, timeoutMs) {
        const env = this._buildExecEnv();
        return new Promise(resolve => {
            cp.exec(cmd, { timeout: timeoutMs, windowsHide: true, maxBuffer: 1024 * 1024, env }, (error, stdout, stderr) => {
    ...
    _buildExecEnv() {
        ...
        if (process.platform === 'win32') {
            ...
            if (env.ProgramFiles)
                extra.push(path.join(env.ProgramFiles, 'nodejs'));
            if (env.LOCALAPPDATA)
                extra.push(path.join(env.LOCALAPPDATA, 'Programs', 'nodejs'));
            const systemRoot = env.SystemRoot || env.WINDIR;
            if (systemRoot)
                extra.push(path.join(systemRoot, 'System32'));
        }
        else {
