...
class StatusPanel {
    ...
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('openclawStatus', 'OpenClaw Status', vscode.ViewColumn.One, { enableScripts: true });
    ...
    async _pollStatus(maxMs, intervalMs) {
    ...
    _ensureGatewayTerminal() {
        if (this._gatewayTerminal)
    ...
    _execFile(command, args, timeoutMs) {
        const env = this._buildExecEnv();
        return new Promise(resolve => {
    ...
    async _runOpenClaw(args, timeoutMs = 30000) {
        if (process.platform !== 'win32') {
        ...
        const mjs = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'npm', 'node_modules', 'openclaw', 'openclaw.mjs');
        // Find node.exe
        let nodeExe;
        const candidates = [
    ...
    _findNodeDir() {
        ...
        if (nvmSymlink && fs.existsSync(path.join(nvmSymlink, 'node.exe')))
            return nvmSymlink;
        ...
        if (nvmHome && fs.existsSync(nvmHome)) {
            ...
            try {
                ...
                for (const entry of entries) {
                    ...
                    if (fs.existsSync(path.join(p, 'node.exe')))
                        return p;
                }
            }
            catch { }
    ...
    async _findOpenClawViaPowerShell() {
    ...
    _parseStatus(out) {
        ...
        if (/rpc probe:\s*failed/.test(lower))
            running = false;
        if (!/rpc probe:\s*(ok|failed)/.test(lower)) {
            ...
            if (/not running|stopped|inactive|down|runtime:\s*stopped/.test(lower))
                running = false;
        }
