...
function readOpenClawConfigRaw(configPath) {
    try {
...
function buildControlCenterData(configPath) {
    ...
    const agents = rawConfig?.agents?.list?.map((a) => ({ id: a.id || "unknown" })) ?? [];
    const channelsConfig = rawConfig?.channels ?? {};
    const channels = Object.entries(channelsConfig).map(([channelKey, channelData]) => {
...
class ConfigPanel {
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel("openclawConfigV2", "OpenClaw Configuration", vscode.ViewColumn.Two, {
            enableScripts: true,
        ...
        this._panel.webview.onDidReceiveMessage((message) => {
            ...
            if (message?.command === "ready") {
                void this._panel.webview.postMessage({
                    command: "init",
                    data: buildControlCenterData((0, config_path_1.resolveConfigPath)()),
                    config: readOpenClawConfigRaw((0, config_path_1.resolveConfigPath)()),
                });
                return;
            }
            ...
            if (message?.command === "openclaw.saveConfig") {
                const configPath = (0, config_path_1.resolveConfigPath)();
            ...
            if (message?.command === "openclaw.channelConfigure") {
                const channelName = String(message?.channel ?? "").trim();
                vscode.window.showInformationMessage("Configure " + (channelName || "channel") + " — edit the config JSON in the Advanced Configuration tab.");
                return;
            }
            ...
            if (message?.command === "openclaw.aiFixConfig") {
                vscode.commands.executeCommand("openclaw.aiFixConfig");
                return;
            }
        });
        void this._update();
    ...
    _setupFileWatcher() {
        ...
        this._fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.Uri.file(path.dirname(configPath)), path.basename(configPath)));
    ...
    _getHtml(data, rawConfig) {
        ...
        const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, "media", "setup-panel.js"));
        const csp = [
            `default-src 'none'`,
        ...
        const connectedChannels = data.channels.filter((channel) => channel.accounts.some((account) => account.status === "connected")).length;
            ...
            <div class="panel">
              <div class="editor-toolbar">
                ...
                <div style="display:flex;gap:8px;">
                  <button class="btn-secondary" id="ai-fix-config">AI Fix</button>
