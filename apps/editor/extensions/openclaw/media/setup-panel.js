(function () {
  ...
  function initUI() {
    ...
    /* AI Fix config */
    var btnAiFix = $("ai-fix-config");
    if (btnAiFix) {
      btnAiFix.addEventListener("click", function () {
        vscode.postMessage({ command: "openclaw.aiFixConfig" });
        showToast("Sending config to AI for review…", "ok");
      });
    }
