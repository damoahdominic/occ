...
const RedoOnboardingButton = ({ className }: { className?: string }) => {
	...
	const voidSettingsService = accessor.get('IVoidSettingsService')
	return <div
		className={`text-void-fg-4 flex flex-nowrap text-nowrap items-center gap-1.5 hover:brightness-110 cursor-pointer ${className}`}
...
export const Settings = () => {
	...
	const [selectedSection, setSelectedSection] =
		useState<Tab>('models');
	const [useOccLegacyModel, setUseOccLegacyModel] = useState(true);
        ...
        `}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<h1 className='text-2xl w-full'>{`OCC Settings`}</h1>
								...
								<ErrorBoundary>
									...
									{!useOccLegacyModel && (
										<div>
											...
											<div className='w-full h-[1px] my-4' />
											<h4 className='text-lg mb-2'>Local Providers</h4>
											...
											<div className='w-full h-[1px] my-4' />
											<h4 className='text-lg mb-2'>Main Providers</h4>
											<VoidProviderSettings providerNames={nonlocalProviderNames.filter(n => n !== 'ocFreeModel')} />
										</div>
									)}
								</ErrorBoundary>
							</div>
