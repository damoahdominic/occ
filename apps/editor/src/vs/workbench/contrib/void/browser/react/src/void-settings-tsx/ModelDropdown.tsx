...
// Human-readable aliases for specific provider+model combinations shown in the UI
...
const getModelDisplayName = (providerName: string, modelName: string): string =>
	MODEL_DISPLAY_ALIASES[providerName]?.[modelName] ?? modelName
...
const ModelSelectBox = ({ options, featureName, className }: { options: ModelOption[], featureName: FeatureName, className: string }) => {
	...
	return <VoidCustomDropdownBox
		...
		getOptionDisplayName={(option) => getModelDisplayName(option.selection.providerName, option.selection.modelName)}
		getOptionDropdownName={(option) => getModelDisplayName(option.selection.providerName, option.selection.modelName)}
		getOptionDropdownDetail={(option) => option.selection.providerName === 'ocFreeModel' ? '' : option.selection.providerName}
