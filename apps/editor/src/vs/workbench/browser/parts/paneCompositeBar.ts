...
export class PaneCompositeBar extends Disposable {
	...
	private createMoveAction(viewContainer: ViewContainer, newLocation: ViewContainerLocation, defaultLocation: ViewContainerLocation): IAction {
		return toAction({
			...
			label: newLocation === ViewContainerLocation.Panel ? localize('panel', "Panel") : newLocation === ViewContainerLocation.Sidebar ? localize('sidebar', "Primary Side Bar") : localize('auxiliarybar', "OCC Side Bar"),
			run: () => {
	...
	private shouldBeHidden(viewContainerOrId: string | ViewContainer, cachedViewContainer?: ICachedViewContainer): boolean {
		...
		// OCcode: permanently remove these panels from the activity bar.
		// Panels can still be opened via keyboard shortcuts / command palette.
		const occAlwaysHidden = new Set([
			'workbench.view.extensions',
			'workbench.view.debug',
			'workbench.view.remote',
		]);
		if (occAlwaysHidden.has(viewContainerId)) {
			return true;
		}
