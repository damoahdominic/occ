...
AppPublisher=HITL, Inc
AppPublisherURL=https://openclawcode.org/
AppSupportURL=https://openclawcode.org/
AppUpdatesURL=https://openclawcode.org/
...
WizardSmallImageFile="{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp,{#RepoDir}\resources\win32\inno-occode.bmp"
SetupIconFile={#RepoDir}\resources\win32\occode.ico
...
// to shutdown OCcode. We rely on the fact that the update signals
// that OCcode is ready to be shutdown, so we're good to use `force` here.
...
begin
  Result := True;
    ...
    if not WizardSilent() and IsAdmin() then begin
      if MsgBox('This User Installer is not meant to be run as an Administrator. If you would like to install OCcode for all users in this system, download the System Installer instead from https://openclawcode.ai. Are you sure you want to continue?', mbError, MB_OKCANCEL) = IDCANCEL then begin
        Result := False;
      end;
    end;
  #endif
...
// OCcode will create a flag file before the update starts (/update=C:\foo\bar)
