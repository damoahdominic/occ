...
- [ ] **T1.1**: Create `errorReporting.ts` with `ErrorReporter` class
...
- [ ] **T1.2**: Create `errorModal.tsx` React component
...
- [ ] **T3.1**: Implement `sendErrorReport(error, logs, systemInfo)`
...
- [ ] **T3.2**: Backend API endpoint (if not already available)
...
- [ ] **T4.1**: E2E test - Docker setup failure flow
